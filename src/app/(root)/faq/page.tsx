                ...
                <div className="max-w-md mx-auto mb-12">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-lg bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            value={searchTerm}
                ...
                {searchTerm === '' && selectedCategory === 'all' && (
                    <div className="mb-12">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            {popularFAQs.map((faq) => (
                                <Card key={`popular-${faq.id}`} className="p-4 hover:shadow-lg transition-all duration-300 cursor-pointer border-l-4 border-l-blue-500 bg-white/80 backdrop-blur-sm" onClick={() => toggleItem(faq.id)}>
                                    <div className="flex items-center justify-between">
                                        <h5 className="font-semibold text-slate-900 pr-4">{faq.question}</h5>
                                        {openItems.includes(faq.id) ? <ChevronUp className="h-4 w-4 text-slate-500" /> : <ChevronDown className="h-4 w-4 text-slate-500" />}
                                    </div>
                ...
                <div className="max-w-4xl mx-auto space-y-4">
                    {filteredFAQs.length > 0 ? (
                        filteredFAQs.map((faq) => (
                            <Card key={faq.id} className="overflow-hidden hover:shadow-lg transition-all duration-300 bg-white/80 backdrop-blur-sm border-0 shadow-md">
                                <Collapsible open={openItems.includes(faq.id)} onOpenChange={() => toggleItem(faq.id)}>
                                    <CollapsibleTrigger asChild>
                                        <CardContent className="p-6 cursor-pointer hover:bg-slate-50/50 transition-colors">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    {faq.popular && <Star className="h-4 w-4 text-yellow-500 fill-current flex-shrink-0" />}
                                                    <h3 className="text-lg font-semibold text-left pr-4 text-slate-900">{faq.question}</h3>
                                                </div>
                                                {openItems.includes(faq.id) ?
                                    ...
                                    <CollapsibleContent>
                                        <CardContent className="pt-0 pb-6 px-6">
                                            <div className="bg-slate-50/50 rounded-lg p-4 mt-2">
                        ...
                        <Card className="p-12 text-center bg-white/80 backdrop-blur-sm">
                            ...
                            <h3 className="text-xl font-semibold mb-2 text-slate-900">No questions found</h3>
...
export default FAQPageEnhanced;